...
class Cross_Attention_Head(nn.Module):
  ...
  def forward(self, q_logits, kv_logits):

    batch_size = q_logits.shape[0]
    q_seq_len = q_logits.shape[1]
    kv_seq_len = kv_logits.shape[1]

    q = self.q_proj(q_logits).view(batch_size, q_seq_len, self.num_heads, self.head_size).transpose(1, 2)
    k = self.k_proj(kv_logits).view(batch_size, kv_seq_len, self.num_heads, self.head_size).transpose(1, 2)
    v = self.v_proj(kv_logits).view(batch_size, kv_seq_len, self.num_heads, self.head_size).transpose(1, 2)
